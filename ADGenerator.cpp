...
struct ADGenerator : Module {
    ...
    void process(const ProcessArgs& args) override {
        ...
        for (int i = 0; i < 3; ++i) {
            ...
            float attackParam, decayParam, curveParam;
            
            if (bpfEnabled[i]) {
                // BPF 開啟時，使用新版參數結構 (i * 6)
                attackParam = params[TRACK1_ATTACK_PARAM + i * 6].getValue();
                decayParam = params[TRACK1_DECAY_PARAM + i * 6].getValue();
                curveParam = params[TRACK1_CURVE_PARAM + i * 6].getValue();
            } else {
                // BPF 關閉時，使用舊版參數結構 (i * 3) - 跳過 AUTO_ROUTE_PARAM
                attackParam = params[TRACK1_ATTACK_PARAM + i * 3].getValue();
                decayParam = params[TRACK1_DECAY_PARAM + i * 3].getValue();
                curveParam = params[TRACK1_CURVE_PARAM + i * 3].getValue();
            }
